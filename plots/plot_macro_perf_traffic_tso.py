...
def plot_macro_common(ax_low, ax_high, postfix, link, low_ax_thres, high_ax_thres):
    ...
    data = pd.read_csv("macro_perf_traffic_tso.csv")
    ...
    yyp.plot_broken_bars(ax_low, ax_high, Xs, Ys, 'Applications', 'Normalized Time' if postfix == 'performance' else 'Normalized Traffic',
                  syss, use_high_axs, xtick_labels=apps, xtick_rotate=30,
                  width=0.18,
                  low_ymin=0.8 if postfix == 'performance' else 0.2, low_ymax=low_ax_thres,
                  high_ymin=high_ax_thres, high_ymax=5.1 if postfix == 'performance' else 4.4,
                  subplot_title=link if postfix == 'performance' else None,
                  show_xlabels=False,
                  show_xticks=(postfix != 'performance'),
                  show_ylabels=(link=='CXL'), ylabel_pos=[0, 0.7],
                  show_yticks=(link=='CXL'), _yticks=[0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1, 1.1, 1.2, 1.3, 1.4, 1.5, 1.6, 1.7] if postfix == 'performance' else [0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6],
                  high_yticks = [2, 3, 4, 5] if postfix == 'performance' else [2.0, 3.0, 4.0],
                  ytick_font_size=13,
                  to_bottom=False,
                  ygrid=True,
...
if __name__ == '__main__':
    fig, axs = yyp.open_subplots(2, 2, fig_size_2sub1, height_ratios=[1, 2.8])
    plot_macro_common(ax_low=axs[1, 0], ax_high=axs[0, 0], postfix='performance', link='CXL', low_ax_thres=1.6, high_ax_thres=1.8)
    plot_macro_common(ax_low=axs[1, 1], ax_high=axs[0, 1], postfix='performance', link='UPI', low_ax_thres=1.6, high_ax_thres=1.8)
    yyp.set_fig_legend(fig, axs[0], font_size=14, pos=[0.327, 0.87])
    fig.subplots_adjust(wspace=0)
    # yyp.close_subplots(f'../../figures/eval_macro_performance_tso.pdf')
    yyp.close_subplots(f'../figures/Figure 13 (top).pdf')
    
    fig, axs = yyp.open_subplots(2, 2, fig_size_2sub2, height_ratios=[1, 5])    
    ...
    fig.subplots_adjust(wspace=0)
    # yyp.close_subplots(f'../../figures/eval_macro_traffic_tso.pdf')
    yyp.close_subplots(f'../figures/Figure 13 (bottom).pdf')