    ...
    if (t == 0) {
        ...
        if (n == root_nid) {
            for (int nid = 0; nid < n_nodes; ++nid) {
                ...
                datass[t][nid] = (char*)mmap(NULL, data_buf_size * n_threads, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
                if (datass[t][nid] == MAP_FAILED) {
                    std::cerr << "Error: mmap failed for data buf nid:" << n << std::endl;
                    return;
                }
                memset((void*)datass[t][nid], 0, data_buf_size * n_threads);
                
                if (test_result) {
                    volatile float* data_flt = reinterpret_cast<volatile float*>(datass[t][nid]);
                    for (int j = 0; j < n_size * n_threads; ++j) {
                        data_flt[j] = 1.0 * (float)(j + 1);
                    }
                }

                doness[t][nid] = (wordT*)mmap(NULL, flag_size * n_threads, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
                if (doness[t][nid] == MAP_FAILED) {
                    std::cerr << "Error: mmap failed for done nid:" << n << std::endl;
                    return;
                }
    #ifndef NO_GEM5
                appl::register_strel((uint64_t)(doness[t][nid]), ((uint64_t)(doness[t][nid])) + flag_size * n_threads);
    #endif
                memset((void*)doness[t][nid], 0, flag_size * n_threads);

                for (int tid = 0; tid < n_threads; ++tid) {
                    datass[tid][nid] = datass[0][nid] + data_buf_size * tid;
                    doness[tid][nid] = doness[0][nid] + flag_size * tid / sizeof(wordT);
                }
            }
            
        } else {

            recv_bufss[t][n] = (char*)mmap(NULL, recv_buf_size * n_threads, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
            if (recv_bufss[t][n] == MAP_FAILED) {
                std::cerr << "Error: mmap failed for recv buf nid:" << n << std::endl;
                return;
            }
#ifndef NO_GEM5
            appl::register_stnt((uint64_t)(recv_bufss[t][n]), ((uint64_t)(recv_bufss[t][n])) + recv_buf_size * n_threads);
#endif
            memset((void*)recv_bufss[t][n], 0, recv_buf_size * n_threads);

            doorbellss[t][n] = (wordT*)mmap(NULL, flag_size * n_threads, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
            if (doorbellss[t][n] == MAP_FAILED) {
                std::cerr << "Error: mmap failed for doorbell nid:" << n << std::endl;
                return;
            }
#ifndef NO_GEM5
            appl::register_strel((uint64_t)(doorbellss[t][n]), ((uint64_t)(doorbellss[t][n])) + flag_size * n_threads);
#endif
            memset((void*)doorbellss[t][n], 0, flag_size * n_threads);

            for (int tid = 1; tid < n_threads; ++tid) {
                recv_bufss[tid][n] = recv_bufss[0][n] + recv_buf_size * tid;
                doorbellss[tid][n] = doorbellss[0][n] + flag_size * tid / sizeof(wordT);
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    for (int n = 0; n < n_nodes; ++n) {
        for (int t = 0; t < n_threads; ++t) {
            if (n == 0 && t == 0)
                continue;
            workers.emplace_back(run_scatter, n, t);
        }
