...
def get_next_file_id(old_file_id):
    if old_file_id == 129: # end TQH
        return 215 # HSTI
    elif old_file_id == 218: # end HSTI
        return 231 # TRNS
    elif old_file_id == 234: # end TRNS
        return 130 # TQH-next
    elif old_file_id == 182: # end TQH
        return 219 # HSTI
    elif old_file_id == 222: # end HSTI
        return 235 # TRNS
    elif old_file_id == 238: # end TRNS
        return 183 # TQH-next
...
if __name__ == "__main__":
            ...
            for sys in syss:
                if app in ('PR', 'SSSP', 'PAD', 'TQH', 'HSTI', 'TRNS'):
                    ...
                    if app == 'CMC-2D':
                        ...
                        if sys != 'WB':
                            traffic = 0.516 * data[f'reduce_256B {sys} {link}']['wt_traffic'] / 2\
                                + 0.455 * data[f'allreduce_8B {sys} {link}']['wt_traffic'] / 2\
                                + 0.029 * data[f'sendrecv_8B {sys} {link}']['wt_traffic'] / 2
                        else:
    ...
    df.to_csv('../plots/macro_perf_traffic.csv', index=False)